...
def setup_module():
    ...
    train_boards_tensor = torch.tensor(train_boards, dtype=torch.float32).unsqueeze(1).repeat(1, 2, 1, 1).to(
        DEVICE)  # (batch, 2, 6, 7)
    ...
    test_boards_tensor = torch.tensor(test_boards, dtype=torch.float32).unsqueeze(1).repeat(1, 2, 1, 1).to(
        DEVICE)  # (batch, 2, 6, 7)
